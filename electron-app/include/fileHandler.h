...
public:
	...
	*/
	...
	static void storeKey(unsigned char* key, const int& keySize, const std::string& outputPath);
	...
	// Creates directory in Downloads and store in parameter
	static bool createRootDir(std::string& tempName);
	...
	static std::string parsePath(const std::string& filePath, const std::string& path, const std::string& newName);
