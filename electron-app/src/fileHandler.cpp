...
void fileHandler::storeKey(unsigned char* key, const int& keySize){
    ...
    std::uniform_int_distribution<> distr(0, 61); // Range: [0, 61]
...
// Mac/Linux
	...
	outputPath = targetFolder+"_key_";

	outputPath += (keySize == 128) ? "128_" : (keySize == 192) ? "192_" : "256_";
	...
	std::cout<<std::endl;
	std::cout<<"Key used: "<<outputPath<<std::endl;
	std::cout<<std::endl;

	keyOutput.close();
}

// Store key with custom output path
void fileHandler::storeKey(unsigned char* key, const int& keySize, const std::string& outputPath){
	...
	std::cout<<"Created key with a custom path"<<std::endl;

	keyOutput.close();
}

// create target directory
bool fileHandler::createRootDir(std::string& tempName){
...
// windows
	...
	// construct path
	std::string targetFolder = std::string(homeDir) + "\\Downloads\\target" + "_" + res + "\\";
...
// Mac/Linux
	...
	// create new folder to store file 
	std::string targetFolder = std::string(homeDir) + "/Downloads/target" + "_" + res + "/";
...
std::string fileHandler::parsePath(const std::string& filePath, const std::string& path, const std::string& newName){

	std::string relativePath = filePath.substr(path.size()-1);


	return newName+relativePath;
}
...
void fileHandler::AES_GCM_DECRYPTION(const std::string& path, unsigned char* key, const int& keySize, bool authTag, bool AD){
	...
	// Authenticate Tag
	if(authTag){
		std::cout<<"---- CHECK AUTHENTICATION TAG"<<std::endl;
